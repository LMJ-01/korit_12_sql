...
- 예를 들어 10행인 테이블과 20행인 테이블을 CROSS JOIN하면 결과 테이블은 10 * 20 인 200행이 됩니다. 그리고 이외의 경우의 수는 존재하지 않습니다.

- 모든 경우의 수를 만들어내기 떄문에 ON 조건을 설정할 필요가 없음.

## JOIN에 대한 전반적인 정리
- 두 테이블의 행을 서로 조합하는 과정인데, 여러 가지(LEFT, RIGHT, INNER, CROSS, FULL OUTER)가 존재하면서, ON 조건을 활용하여 전체 경우의 수에서 어떤 행만 가져올 수 있을지를 정할 수 있다.

- FULL OUTER JOIN의 경우 CROSS JOIN의 경우 CROSS JOIN과 차이가 존재하는데, LEFT JOIN의 결과값과 RIGHT JOIN의 결과값을 _중복 없이_ 결합하는 개념으로, 합집합에 해당함. 전체를 구하는 컨셉을 유사하나 합집합이 전체 경우의 수를 의미하는 것은 아니기 떄문에 구분해서 사용할 필요가 있습니다. CROSS JOIN과 달리 ON 조건을 요구합니다.

- 운영 환경에서는 CROSS JOIN을 제한하는 편입니다. 전체 경우의 수를 구하는데 실제로 필요한 경우는 거의 없기 떄문입니다.
...
```sql
SELECT u.country AS 국가, COUNT(DISTINCT o.id) AS 주문건수
...
```sql
SELECT o.user_id AS 회원아이디, ROUND(SUM(p.price * oo.quantity), 2) AS 정상가격총합, ROUND(SUM(p.discount_price * oo.quantity), 2) AS 할인금액총합
    FROM orders o LEFT JOIN orderdetails oo ON o.id = oo.order_id INNER JOIN products p ON oo.product_id = p.id
	 GROUP BY o.user_id
...
5. 이하의 조건에 해당하는 테이블이 있다고 가정했을 떄 질문에 답하시오.
    - 왼쪽 테이블 A : 컬럼 개수 5 개 / 150 행
    - 오른쪽 테이블 B : 컬럼 개수 7 개 / 100 행
    - 두 테이블은 공통 키 값 컬럼을 1 개 보유 위 조건의 두 테이블을 CROSS/LEFT/RIGHT/INNER JOIN으로 결합했을 떄 결과 테이블의 행과 열 개수를 계산하시오(SELECT * 기준)

CROSS 행과 열 개수 : 15000 행 12 열

LEFT 행과 열 개수 :  최소 150, 최대 15000 행  12 열

RIGHT 행과 열 개수 : 최소 100, 최대 15000 행 행 12 열

INNER 행과 열 개수 : 최소 0행 최대 15000 행 12 열
...
좌측 상단을 확인했을 때 결과로 표시되는 것으로 보아 users를 그대로 가지고 온 것이 아니라는 것을 알 수 있습니다. 이는 중복제거가 적용된 결과값에 해당합니다.
...
- UNION 및 ALL을 적용했을 떄 SELECT문을 쓸 떄 full scan한다면 상관없지만 특정 컬람만 보여주겠다고 한다면 첫 번쨰 테이블에서 컬럼을 편집한 것과 동일하게 두 번 쨰 테이블도 수정해야 합니다.
...
그래서 지사에서 올라온 각 테이블의 row들을 통합할 떄 본사에서 UNION을 쓸 수 있겠네요.
실무적으로 UNION보다는 UNION ALL을 더 권장합니다. 대량의 데이터에서 중복 항목을 제거할 떄 부하가 일어날 수 있기 떄문에 일단 중복되더라도 보여주는 것을 좀 더 선호하고, 그 후에 조건절 등을 통해 분류합니다.
...
### 연습문제
1. orders에서 주문일자가 2015년 10월인 건과 2015sus 12월인 건을 SELECT로 각각 추출하고 UNION ALL을 사용해 하나로 결합하시오. 단, 최종 결과는 최신 순으로 정렬하시오.

```sql
SELECT order_date
    FROM orders
    WHERE order_date Like '2015-10%'
UNION ALL
SELECT order_date
    FROM orders
    WHERE order_date Like '2015-12%'
...
```sql
SELECT id AS 아이디, phone AS 연락처, country AS 거주국가, city AS 거주도시, is_marketing_agree AS 마케팅수신동의여부 
    FROM users
...
```sql
SELECT *
    FROM orderdetails LEFT JOIN products p
    ON o.product_id = p.id
UNION
SELECT *
    FROM orderdetails RIGHT JOIN products p
    ON o.product_id = p.id;
```

## 서브 쿼리(Sub Query)

- JOIN / UNION은 각각 좌우 / 상하로 두 테이블 결합하는 방법이었습니다. 즉 이미 존재하는 테이블에 접근하여 결합을 시도 했다는건데, 내가 작성한 쿼리를 실행하여 나온 결과값인 테이블을 다시 사용하거나 조건으로 쓰거나, 혹은 값으로 쓸 수 있을까와 관련된 부분이 서브쿼리입니다.
...
- SQL에서는 직접 작성한 쿼리에서 나온 결과값을 마치 테이블처럼 사용할 수 있는 기능이 있는데, 이를 서브 쿼리라고 하며, 일반 SELECT문보다 먼저 연산이 되어야만 하기 떄문에 `소괄호()` 로 묶어서 SQL문을 작성합니다(아까 UNION할 떄 잠깐 써봤습니다). 해당 경우 실제 테이블은 아니지만 결과값을 테이블처럼 사용할 수 있습니다.

- 그런데 시험에 나올 떄 sub query의 종류도 물어봅니다. 이는 서브쿼리가 어느 위치에서 사용되어지느냐에 따라서 결정지어집니다.
...
즉 어차피 서브 쿼리 내의 결곽밧이 38.8이므로

```sql
SELECT name, price, 38.8 AS 평균가격
FROM products;
```
과 동일한 결곽밧이 나온다는 것을 알 수 있습니다. 그러면 계산해놓고 그대로 값 넣으면 되겠네요라고 생각할 수도 있을 것 같은데 어차피 중요한건 서브쿼리의 결과값이 단일값인 경우에 SELECT절에 들어갈 수 있다는 개념입니다.
...
평균각격이 출력되기는 하지만 나머지 row들이 다 날아갑니다. 그 이유는 AVG(price)의 결과가 하나의 row에 해당되기 때문입니다.

- 이상의 과정에서 스칼라 서브쿼리를 작성할 떄 _단일 값_ 이 반환되도록 작성해야한다는 점에서 유의해야 컬럼으로 추가하고, 각 행마다 붙이고 싶다면 서브 쿼리를 두 개로 나누어서 작성해야합니다.
...
- 참조 사항 : subquery를 from 절에 집어넣었을 떄 서브쿼리의 연산 결과값이 테이블로 사용된다는 것을 확인했습니다. 해당 경우 a를 명시하지 않으면 오류가 발생했습니다. 이는 약칭에 해당합니다(JOIN에서 썼습니다). 서브쿼리의 결과값을 마치 변수에 대입하는 것으로 간주하시면 되겠습니다. 명시 안하면 연산 결과가 변수에 저장이 되지 않기 떄문에 걔를 기준으로 main query의 연산을 불러올 수 없습니다.
...
- 참조 사항 : WHERE에서 IN 연산자와 함께 서브 쿼리를 활용할 경우 서브쿼리에서 출력할 컬럼 개수와 필터링 적용 대상 컬럼의 개수가 일치해야 합니다. 저희는 staff_id라는 컬럼 하나만 있었기 떄문에, 서브 쿼리 내에서 staff 테이블의 id 컬럼의 값들만 출력하도록 개수를 일치시켰습니다.

```sql
SELECT *
    FROM orders
    WHERE (staff_id,user_id) IN (SELECT id, user_id
                        FROM staff
                        WHERE last_name in ('kyle','Scott'))
```
...
- 문제 : products에서 할인 가격이 가장 비싼 제품정보를 출력하시오.
...
SELECT * 
    ...
    (SELECT * 
        FROM orders
        WHERE order_date LIKE '2015-07%') o
    INNER JOIN 
    (SELECT *
        FROM orderdetails
        WHERE quantity > 49) od 
	ON o.id = od.order_id;
...
```sql
SELECT *
	FROM (SELECT country,COUNT(DISTINCT id) AS 국가별회원수 FROM users GROUP BY country) u
	WHERE 국가별회원수 >  4
